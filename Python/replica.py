"""
In a nutshell...
Give the script things (preferably without ASCII characters outside of the alphabet) and it will generate things like it.
Uses modern transformer AI tech
Its my attempt at a model that can replicate data to train other models
(so its not production standard nor capable of replicating any sort of data you give it)

Notes about that model in contrast to large scale production models (ex: minGPT)
- Did not include dropout layers because the models we train here are small,
  it's not necessary at this stage and at this scale.
- Did not include weight decay and all of the complexity around what parameters are
...
References:
    ...
    CNN: https://arxiv.org/abs/1609.03499
...
# -----------------------------------------------------------------------------
# Transformer Language Model (very similar to what is used in GPT-2)

class NewGELU(nn.Module):
    """ Implementation of the GELU activation function similar to OpenAI GPT). """
...
class CausalSelfAttention(nn.Module):
    ...
    It's 'possible to use torch.nn.MultiheadAttention here but I chose not to. (For learning purposes)
    """
...
class RNNCell(nn.Module):
    """
    Takes input at current time step x_{t} and the hidden state at the
...
if __name__ == '__main__':
    ...
    # system/input/output
    parser.add_argument('--input-file', '-i', type=str, default='data/words.txt',
